...
- 2026-06-23: Direct HTTP calls to `cocos-mcp` were verified as a reliable fallback when the MCP tool is not auto-loaded into the Codex thread. In this project, `http://127.0.0.1:3603/mcp` accepted `tools/list` and `tools/call` JSON-RPC requests directly, and scene writes succeeded with revision-gated mutations plus `cocos_scene_save`. This fallback should be treated as the first recovery path before attempting manual scene edits.
- 2026-06-23: `cocos_component_event_bind` for `cc.Button` expects `eventName: "click"` rather than `clickEvents`. It also rejects empty `handlerComponentType`, even when the editor already resolves the component internally. This is workable once known, but the current error path is easy to hit during automation.
- 2026-06-23: After adding a new method to a TypeScript component, the editor-side component metadata may lag behind the source file. `cocos_script_refresh` resolved this and then allowed `cocos_component_event_bind` to find the new method correctly. This should be part of the default workflow whenever a just-added method or property is reported missing by MCP.
